export class Asynchronous {
  ...
  public start = async () => {
    ...
    const data = await this.fetch_hello_2step_return().then((res: Response) =>
      res.json()
    );
    console.log(data);
    console.log("----------------------");
    ...
    console.log(
      "try_async_awaitの開始 戻り値としてPromise<void>なので1thenが可能"
    );
    await this.try_async_await()
    ...
    await this.return_async_await().then((res: Response) => console.log(res));
    console.log("----------------------");
